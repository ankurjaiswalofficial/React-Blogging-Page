...
export default function Card({tagName, tagColor, title, description, creatorName, creationDate, creatorImage, thumbnailImage, flatCard=false, type2=false}) {
    return (
        <div className={`flex items-start justify-between ${flatCard ? 'flex-row w-full h-[26rem] my-8' : 'flex-col w-[30%] h-[40rem] border border-gray-300 rounded-2xl '} overflow-hidden`}>
